...
export function CompeteSection({ friendId, friendName, myCardCount, friendCardCount, userId }) {
  ...
  const fetchMyCards = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('user_cards')
      .select('id, rating, player:players(first_name, last_name, face_image, position, age, card_number, team:teams(name, primary_color, text_color, logo_url, card_color, sport))')
      .eq('user_id', userId)
    setMyCards(data || [])
  }, [userId])
